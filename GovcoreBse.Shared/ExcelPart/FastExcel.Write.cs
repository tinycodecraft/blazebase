...
namespace GovcoreBse.Shared.ExcelPart
{
    public partial class FastExcel
    {
        ...
        /// </summary>
        public void Write(DataTable table, string sheetName,bool useheading=true,int excount=0)
        {
            var worksheet = new Worksheet();
            worksheet.PopulateRowsFromDataTable(table, excount,useheading);
            Write(worksheet, null, sheetName, excount);
        }

        public void Write(Dictionary<int, DataTable> tables, string sheetName)
        {
            var worksheet = new Worksheet();
            var allrows = new List<Row>();
            foreach(var tkey in tables.Keys)
            {
                allrows.AddRange( worksheet.GetPopulateRowsFromDataTable(tables[tkey], tkey));
                
            }
            worksheet.Rows = allrows;
            Write(worksheet, null, sheetName, -1);
        }
