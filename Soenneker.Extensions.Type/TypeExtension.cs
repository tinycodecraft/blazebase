...
{
    // Use a non-generic delegate that accepts ReadOnlySpan<char> (ref struct) to avoid CS9244
    private delegate object? SpanParser(ReadOnlySpan<char> span);


    private static SpanParser GetFunc(System.Type type)
    {
        switch (type) {
            case var t when t == typeof(string):
                return static s => s.ToString();
            case var t when t == typeof(int):
                return static s => int.TryParse(s, NumberStyles.Integer, CultureInfo.InvariantCulture, out int v) ? v : null;
            case var t when t == typeof(long):
                return static s => long.TryParse(s, NumberStyles.Integer, CultureInfo.InvariantCulture, out long v) ? v : null;
            case var t when t == typeof(short):
                return static s => short.TryParse(s, NumberStyles.Integer, CultureInfo.InvariantCulture, out short v) ? v : null;
            case var t when t == typeof(ushort):
                return static s => ushort.TryParse(s, NumberStyles.Integer, CultureInfo.InvariantCulture, out ushort v) ? v : null;
            case var t when t == typeof(uint):
                return static s => uint.TryParse(s, NumberStyles.Integer, CultureInfo.InvariantCulture, out uint v) ? v : null;
            case var t when t == typeof(ulong):
                return static s => ulong.TryParse(s, NumberStyles.Integer, CultureInfo.InvariantCulture, out ulong v) ? v : null;
            case var t when t == typeof(byte):
                return static s => byte.TryParse(s, NumberStyles.Integer, CultureInfo.InvariantCulture, out byte v) ? v : null;
            case var t when t == typeof(sbyte):
                return static s => sbyte.TryParse(s, NumberStyles.Integer, CultureInfo.InvariantCulture, out sbyte v) ? v : null;
            case var t when t == typeof(float):
                return static s => float.TryParse(s, NumberStyles.Float | NumberStyles.AllowThousands, CultureInfo.InvariantCulture, out float v) ? v : null;
            case var t when t == typeof(double):
                return static s => double.TryParse(s, NumberStyles.Float | NumberStyles.AllowThousands, CultureInfo.InvariantCulture, out double v) ? v : null;
            case var t when t == typeof(decimal):
                return static s => decimal.TryParse(s, NumberStyles.Number, CultureInfo.InvariantCulture, out decimal v) ? v : null;
            case var t when t == typeof(bool):
                return static s => bool.TryParse(s, out bool v) ? v : null;
            case var t when t == typeof(DateTime):
                return static s => DateTime.TryParse(s, CultureInfo.InvariantCulture, DateTimeStyles.RoundtripKind, out DateTime v) ? v : null;
            case var t when t == typeof(DateTimeOffset):
                return static s => DateTimeOffset.TryParse(s, CultureInfo.InvariantCulture, DateTimeStyles.RoundtripKind, out DateTimeOffset v) ? v : null;
            case var t when t == typeof(TimeSpan):
                return static s => TimeSpan.TryParse(s, CultureInfo.InvariantCulture, out TimeSpan v) ? v : null;
            case var t when t == typeof(Guid):
                return static s => Guid.TryParse(s, out Guid v) ? v : null;
            case var t when t == typeof(Uri ):
                return static s => Uri.TryCreate(s.ToString(), UriKind.RelativeOrAbsolute, out Uri? uri) ? uri : null;
            case var t when t == typeof(char):
                return static s => s.Length == 1 ? s[0] : null;
            case var t when t == typeof(DateOnly):
                return static s => DateOnly.TryParse(s, CultureInfo.InvariantCulture, DateTimeStyles.None, out DateOnly v) ? v : null;
            case var t when t == typeof(TimeOnly):
                return static s => TimeOnly.TryParse(s, CultureInfo.InvariantCulture, DateTimeStyles.None, out TimeOnly v) ? v : null;
